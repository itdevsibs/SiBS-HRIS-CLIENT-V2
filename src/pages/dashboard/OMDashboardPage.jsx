 ...
 * Front-end only.
 *
 * Operation Manager filtering rule:
 * OM can only see data where:
 * 1. role.department === currentUser.department
 * OR
 * 2. role.account is included in currentUser.accounts
 *
 * Replace mockCurrentUser with the logged-in user from your Auth Context/API.
 */
...
function getManagerAccessibleRoles(roles, user) {
            ...
            <div className="flex flex-col gap-5">
              <div className="rounded-xl border border-[#e6ecf2] bg-slate-50 p-5">
                ...
                <div>
                  ...
                  <DetailRow label="Account" value={role.account} />
                  <DetailRow
...
export default function OMDashboardPage() {
  ...
  const managerRoles = useMemo(() => {
    return getManagerAccessibleRoles(dashboardRoles, currentUser);
  }, [currentUser]);

  const filteredRoles = useMemo(() => {
    const keyword = search.trim().toLowerCase();

    if (!keyword) return managerRoles;
  ...
  const recruiterLoads = useMemo(() => {
    ...
    managerRoles.forEach((role) => {
      ...
      grouped[role.taOwner].sourced += role.sourced;
      grouped[role.taOwner].interviewed += role.interviewed;
      grouped[role.taOwner].hired += role.hired;
    });
        ...
        <section className="grid grid-cols-1 gap-4 2xl:grid-cols-[1fr_380px]">
            ...
            <div className="p-5 sm:p-6">
              ...
              <div className="mt-5 flex items-center justify-between gap-4 max-sm:flex-col max-sm:items-stretch">
                <p className="m-0 text-sm font-semibold text-sibs-tertiary-5">
                  Showing 1 to {filteredRoles.length} of {managerRoles.length}{" "}
                  roles
                </p>
          ...
          <aside className="flex flex-col gap-4">
            ...
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-sibs-primary-1/80">
                Dashboard values should be restricted to the Operation Manager’s
                own department and assigned accounts. This should also be
                enforced in the backend API, not only in the frontend.
              </p>
            </div>
          </aside>
        </section>
