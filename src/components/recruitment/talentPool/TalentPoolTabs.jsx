...
const tabs = [
  ...
  {
    key: TALENT_POOL_TABS.NEW_APPLICANT,
    label: "New Applicant",
    icon: UserRoundPlus,
  },
  {
    key: TALENT_POOL_TABS.APPLICANT_PIPELINE,
    label: "Applicant Pipeline",
    icon: ClipboardList,
  },
  { key: TALENT_POOL_TABS.BELOW_18, label: "Below 18", icon: UserRoundX },
  {
    key: TALENT_POOL_TABS.INCOMPLETE_REQUIREMENTS,
    label: "Incomplete Requirements",
    icon: ClipboardList,
  },
  {
    key: TALENT_POOL_TABS.DROP_OFF,
    label: "Drop Off List",
    icon: AlertTriangle,
  },
  {
    key: TALENT_POOL_TABS.LEADS_CONVERTED,
...
export default function TalentPoolTabs({ activeTab, onChange, counts = {} }) {
  const newApplicantCount = Number(
    counts?.[TALENT_POOL_TABS.NEW_APPLICANT] || 0,
  );

  const visibleCounts =
    Number.isFinite(newApplicantCount) && newApplicantCount > 0
      ? {
